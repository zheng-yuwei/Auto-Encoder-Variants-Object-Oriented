...
if option4AE.isSparse
    ...
    cost.costSparse = option4AE.sparseBeta * sum( KL );
else
    cost.costSparse = 0;
end
...
cost.costError = sum( sum((outputX - input).^2) ) / m / 2;
cost.costRegul = 0.5 * option4AE.decayLambda * ( sum(sum(W1 .^ 2)) + sum(sum(W2 .^ 2)) );  

% 求总的cost
cost.cost = cost.costError + cost.costRegul + cost.costSparse;
cost = cost.cost;
